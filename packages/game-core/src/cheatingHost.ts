...
function score(marks: Mark[]): [hits: number, presents: number] {
  let h = 0, p = 0;
  for (const m of marks) { if (m === 'hit') h++; else if (m === 'present') p++; }
  return [h, p];
}
...
export function nextCheatingCandidates(candidates: string[], guess: string) {
  ...
  for (const ans of candidates) {
    ...
    const key = marks.map(m => (m==='hit'?'O':m==='present'?'?':'_')).join('');
    const b = buckets.get(key) ?? { marks, words: [], s: score(marks) };
    b.words.push(ans);
    buckets.set(key, b);
  }
  const chosen = [...buckets.values()].sort((a,b)=>a.s[0]-b.s[0] || a.s[1]-b.s[1])[0];
